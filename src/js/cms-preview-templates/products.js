  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="bg-off-white pv4">
          <div className="ph3 mw7 center">
            ...
            <div className="flex-ns flex-wrap mhn2-ns mb3">
              {(entry.getIn(["data", "intro", "blurbs"]) || []).map(
                (blurb, index) => (
        ...
        <div className="mw7 center ph3 pv4">
          <div className="flex flex-wrap mhn1">
            <div className="w-100 w-50-ns ph1-ns">
              <img
                src={getAsset(entry.getIn(["data", "main", "image1", "image"]))}
              />
            </div>

            <div className="w-100 w-50-ns ph1-ns">
              <img
                src={getAsset(entry.getIn(["data", "main", "image2", "image"]))}
              />
            </div>

            <div className="w-100 ph1-ns">
              <img
                src={getAsset(entry.getIn(["data", "main", "image3", "image"]))}
              />
            </div>
          </div>
        </div>
        ...
        <img
          src={getAsset(entry.getIn(["data", "full_image"]))}
          alt=""
          className="db w-100"
        />
        ...
        <div className="bg-off-white pv4 ph3">
          <div className="mw7 center">
            ...
            <div className="flex-ns mhn2-ns mw7">
              {(entry.getIn(["data", "pricing", "plans"]) || []).map(
                (plan, index) => (
                  <div className="w-33-ns ph2" key={index}>
                    <div className="ph2">
                      <h3 className="b f5 grey-3 tc lh-title mb3">
                        {plan.get("plan")}
                      </h3>
                      ...
                      <ul>
                        {(plan.get("items") || []).map((item, index) => (
                          <li key={index}>
                            <p
                              className={
                                index + 1 !== plan.get("items").size
                                  ? "pb2 mb2 divider-grey"
                                  : null
                              }
                            >
                              {item}
